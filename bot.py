...
import config  # your config.py with BOT_TOKEN, MONGODB_URI, COOKIES_FILE
import json
...
def get_audio_stream_info(ffprobe_json):
    ...
    for s in streams:
        if s.get("codec_type") == "audio":
            ...
            audio_streams.append({"index": idx, "label": label})
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if filesize > MAX_TELEGRAM_FILESIZE:
            await msg.edit_text(
                "Downloaded file is too large for Telegram upload (over 2GB)."
            )
            return

        await msg.edit_text(
            f"Downloaded successfully: {os.path.basename(filename)}\nAnalyzing audio tracks..."
        )
        ...
        if not audio_streams:
            await update.message.reply_document(
                document=open(filename, "rb"), filename=os.path.basename(filename)
            )
            await msg.delete()
            os.remove(filename)
        ...
        buttons.append(
            [InlineKeyboardButton(text="Upload Full Video", callback_data=f"fullvideo|{filename}")]
        )
        ...
        if action == "fullvideo":
            ...
            try:
                with open(out_audio, "rb") as f:
                    await query.message.reply_document(
                        document=f, filename=os.path.basename(out_audio)
                    )
